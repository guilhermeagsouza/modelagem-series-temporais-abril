...
fit.air <- forecast::Arima(
  serie_treino, 
  order = c(2,0,0), seasonal = c(0,0,0),
  method = "ML",
  lambda = 0
  #xreg = eventos_treino
)
...
data.frame(previsao_sarima = previsao$mean %>% as.numeric()) %>% 
  readr::write_rds("previsao-ajustados-sarima/guiadoestudante.Rda")
...
data_previsao <- readr::read_rds("previsao-sessoes-sites/tbl_previsao_guiadoestudante.Rda") %>%
...
data.frame(.index = data_previsao, 
  ...
  readr::write_rds("previsao-sessoes-sites/tbl_sarima_guiadoestudante.Rda")
