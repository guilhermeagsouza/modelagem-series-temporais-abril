...
ts_serie_capricho <-  stats::ts(
  data = serie_capricho$value,
  start = c(2015,01), 
  frequency = 365.25/7
)

ano_mes_dia_final <- serie_capricho$date %>% dplyr::last() %>% lubridate::ymd()
ano_mes_dia_final
...
# Preciso
ts_treino <- stats::ts(data = serie_capricho[1:(nrow(serie_capricho)-h_horizonte),]$value,
                       start = c(2015,01), frequency = 365.25/7)
teste <- serie_capricho[(nrow(serie_capricho)-(h_horizonte-1)):nrow(serie_capricho),]$value
...
ano_mes_dia_teste <- serie_capricho[1:(nrow(serie_capricho)-h_horizonte),]$date %>% 
...
previsao_treino %>% 
  data.frame() %>% 
  readr::write_rds("dados-ajustados-teste/previsao-treino-stlf-capricho.Rda")
...
previsao_final <- ts_serie_capricho %>% 
...
previsao_final %>% readr::write_rds("previsao-modelos-stlf/previsao_capricho.Rda")
