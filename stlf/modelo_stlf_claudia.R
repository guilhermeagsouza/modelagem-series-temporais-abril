...
ts_serie_claudia <-  stats::ts(
  data = serie_claudia$value,
  start = c(2017,01), 
  frequency = 365.25/7
)

ano_mes_dia_final <- serie_claudia$date %>% dplyr::last() %>% lubridate::ymd()
ano_mes_dia_final
...
# Preciso
ts_treino <- stats::ts(data = serie_claudia[1:(nrow(serie_claudia)-h_horizonte),]$value,
                       start = c(2017,01), frequency = 365.25/7)
teste <- serie_claudia[(nrow(serie_claudia)-(h_horizonte-1)):nrow(serie_claudia),]$value
...
ano_mes_dia_teste <- serie_claudia[1:(nrow(serie_claudia)-h_horizonte),]$date %>% 
...
previsao_treino %>% 
  data.frame() %>% 
  readr::write_rds("dados-ajustados-teste/previsao-treino-stlf-claudia.Rda")
...
previsao_final <- ts_serie_claudia %>% 
...
previsao_final %>% readr::write_rds("previsao-modelos-stlf/previsao_claudia.Rda")
