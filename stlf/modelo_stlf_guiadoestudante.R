...
ts_serie_guiadoestudante <-  stats::ts(
  data = serie_guiadoestudante$value,
  start = c(2015,01), 
  frequency = 365.25/7
)

ano_mes_dia_final <- serie_guiadoestudante$date %>% dplyr::last() %>% lubridate::ymd()
ano_mes_dia_final
...
# Preciso
ts_treino <- stats::ts(data = serie_guiadoestudante[1:(nrow(serie_guiadoestudante)-h_horizonte),]$value,
                       start = c(2015,01), frequency = 365.25/7)
teste <- serie_guiadoestudante[(nrow(serie_guiadoestudante)-(h_horizonte-1)):nrow(serie_guiadoestudante),]$value
...
ano_mes_dia_teste <- serie_guiadoestudante[1:(nrow(serie_guiadoestudante)-h_horizonte),]$date %>% 
...
previsao_treino %>% 
  data.frame() %>% 
  readr::write_rds("dados-ajustados-teste/previsao-treino-stlf-guiadoestudante.Rda")
...
previsao_final <- ts_serie_guiadoestudante %>% 
...
previsao_final %>% readr::write_rds("previsao-modelos-stlf/previsao_guiadoestudante.Rda")
