...
ts_serie_boaforma <-  stats::ts(
  data = serie_boaforma$value,
  start = c(2017,01), 
  frequency = 365.25/7
  )

ano_mes_dia_final <- serie_boaforma$date %>% dplyr::last() %>% lubridate::ymd()
ano_mes_dia_final
...
# Preciso
ts_treino <- stats::ts(data = serie_boaforma[1:(nrow(serie_boaforma)-h_horizonte),]$value,
                       start = c(2017,01), frequency = 365.25/7)
teste <- serie_boaforma[(nrow(serie_boaforma)-(h_horizonte-1)):nrow(serie_boaforma),]$value
...
#Previsão com os dados de treino num horizonte de 16 semanas à frente
ano_mes_dia_teste <- serie_boaforma[1:(nrow(serie_boaforma)-h_horizonte),]$date %>% 
...
previsao_treino %>% 
  data.frame() %>% 
  readr::write_rds("dados-ajustados-teste/previsao-treino-stlf-boaforma.Rda")
...
previsao_final <- ts_serie_boaforma %>% 
...
previsao_final %>% readr::write_rds("previsao-modelos-stlf/previsao_boaforma.Rda")
