...
ts_serie_superinteressante <-  stats::ts(
  data = serie_superinteressante$value,
  start = c(2015,01), 
  frequency = 365.25/7
)

ano_mes_dia_final <- serie_superinteressante$date %>% dplyr::last() %>% lubridate::ymd()
ano_mes_dia_final
...
# Preciso
ts_treino <- stats::ts(data = serie_superinteressante[1:(nrow(serie_superinteressante)-h_horizonte),]$value,
                       start = c(2015,01), frequency = 365.25/7)
teste <- serie_superinteressante[(nrow(serie_superinteressante)-(h_horizonte-1)):nrow(serie_superinteressante),]$value
...
ano_mes_dia_teste <- serie_superinteressante[1:(nrow(serie_superinteressante)-h_horizonte),]$date %>% 
...
previsao_treino %>% 
  data.frame() %>% 
  readr::write_rds("dados-ajustados-teste/previsao-treino-stlf-superinteressante.Rda")
...
previsao_final <- ts_serie_superinteressante %>% 
...
previsao_final %>% readr::write_rds("previsao-modelos-stlf/previsao_superinteressante.Rda")
