...
ts_serie_quatrorodas <-  stats::ts(
  data = serie_quatrorodas$value,
  start = c(2015,01), 
  frequency = 365.25/7
)

ano_mes_dia_final <- serie_quatrorodas$date %>% dplyr::last() %>% lubridate::ymd()
ano_mes_dia_final
...
# Preciso
ts_treino <- stats::ts(data = serie_quatrorodas[1:(nrow(serie_quatrorodas)-h_horizonte),]$value,
                       start = c(2015,01), frequency = 365.25/7)
teste <- serie_quatrorodas[(nrow(serie_quatrorodas)-(h_horizonte-1)):nrow(serie_quatrorodas),]$value
...
ano_mes_dia_teste <- serie_quatrorodas[1:(nrow(serie_quatrorodas)-h_horizonte),]$date %>% 
...
previsao_treino %>% 
  data.frame() %>% 
  readr::write_rds("dados-ajustados-teste/previsao-treino-stlf-quatrorodas.Rda")
...
previsao_final <- ts_serie_quatrorodas %>% 
...
previsao_final %>% readr::write_rds("previsao-modelos-stlf/previsao_quatrorodas.Rda")
