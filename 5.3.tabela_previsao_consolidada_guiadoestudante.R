...
tbl_teste <- readr::read_rds("dados-modelo-teste/guiadoestudante.Rda") 

STLF_teste <- readr::read_rds("dados-ajustados-teste/previsao-treino-stlf-guiadoestudante.Rda") %>% 
  dplyr::rename(ATUAL = REAL, STLF = PREVISAO_STLF)

SARIMA_teste <- readr::read_rds("dados-ajustados-teste/previsao-treino-sarima-guiadoestudante.Rda") %>% 
  dplyr::rename(ATUAL = REAL, SARIMA = PREVISAO_SARIMA)

PROPHET_teste <- readr::read_rds("dados-ajustados-teste/previsao-treino-prophet-guiadoestudante.Rda") %>%
  dplyr::rename(ATUAL = REAL, PROPHET = PREVISAO_PROPHET)

#Carregando os pesos do modelo ENSEMBLE
pesos <- readr::read_rds("previsao-modelos-ensemble/pesos_ensemble_guiadoestudante.Rda")
...
################################################################################
tbl_previsao <- readr::read_rds("previsao-modelos-ensemble/modelo_ensemble_guiadoestudante.Rda") %>% 
  dplyr::mutate(
...
tbl_dados_reais <- readr::read_rds("previsao-sessoes-sites/tbl_previsao_guiadoestudante.Rda") %>% 
...
tbl_teste_previsao %>% 
  ...
  dplyr::arrange(.index) %>% 
  readr::write_rds(
    "tabela_previsao_consolidada/tbl_previsao_consolidada_guiadoestudante.Rda")
