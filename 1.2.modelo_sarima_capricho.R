...
df_base <- readr::read_rds("dados_p_graficos/base_capricho.Rda")
...
fit.air <- forecast::Arima(
  serie_treino, 
  order = c(2,1,1), seasonal = c(0,0,0),
  method = "ML",
  lambda = 0
  #xreg = eventos_treino
)
...
data.frame(previsao_sarima = previsao$mean %>% as.numeric()) %>% 
  readr::write_rds("previsao-ajustados-sarima/capricho.Rda")

previsao_capricho <- previsao %>% 
  data.frame() %>% 
  cbind(teste)
...
data_previsao <- readr::read_rds("previsao-sessoes-sites/tbl_previsao_capricho.Rda") %>%
...
data.frame(.index = data_previsao, 
  ...
  readr::write_rds("previsao-sessoes-sites/tbl_sarima_capricho.Rda")
