...
df_base <- readr::read_rds("dados_p_graficos/base_superinteressante.Rda")

horizonte_previsao <- 30

df_base <- df_base %>% 
  dplyr::mutate(quebra_estrutural = fun_break_point(y)) #%>% 
#dplyr::filter(lubridate::year(x) > 2016)
...
fit.air <- forecast::Arima(
  serie_treino, 
  order = c(1,1,0), seasonal = c(0,0,0),
  method = "ML",
  lambda = 0
  #xreg = eventos_treino
)
...
data.frame(previsao_sarima = previsao$mean %>% as.numeric()) %>% 
  readr::write_rds("previsao-ajustados-sarima/superinteressante.Rda")

previsao_superinteressante <- previsao %>% 
  data.frame() %>% 
  cbind(teste)

# Métrica MAPE
previsao_superinteressante%>% 
...
fit.air2 <- forecast::Arima(
  stats::ts(data = df_base$y, frequency = 365.25/7, start = 2015 + 31/365.25),
  order = c(1,1,0), seasonal = c(0,0,0),
  method = "ML",
  lambda = 0
)
...
data_previsao <- readr::read_rds("previsao-sessoes-sites/tbl_previsao_superinteressante.Rda") %>%
...
data.frame(.index = data_previsao, 
  ...
  readr::write_rds("previsao-sessoes-sites/tbl_sarima_superinteressante.Rda")
