...
tbl_teste <- readr::read_rds("dados-modelo-teste/saude.Rda") 

STLF_teste <- readr::read_rds("dados-ajustados-teste/previsao-treino-stlf-saude.Rda") %>% 
  dplyr::rename(ATUAL = REAL, STLF = PREVISAO_STLF)

SARIMA_teste <- readr::read_rds("dados-ajustados-teste/previsao-treino-sarima-saude.Rda") %>% 
  dplyr::rename(ATUAL = REAL, SARIMA = PREVISAO_SARIMA)

PROPHET_teste <- readr::read_rds("dados-ajustados-teste/previsao-treino-prophet-saude.Rda") %>%
  dplyr::rename(ATUAL = REAL, PROPHET = PREVISAO_PROPHET)

#Carregando os pesos do modelo ENSEMBLE
pesos <- readr::read_rds("previsao-modelos-ensemble/pesos_ensemble_saude.Rda")
...
################################################################################
tbl_previsao <- readr::read_rds("previsao-modelos-ensemble/modelo_ensemble_saude.Rda") %>% 
  dplyr::mutate(
...
tbl_dados_reais <- readr::read_rds("previsao-sessoes-sites/tbl_previsao_saude.Rda") %>% 
...
tbl_teste_previsao %>% 
  ...
  dplyr::arrange(.index) %>% 
  readr::write_rds(
    "tabela_previsao_consolidada/tbl_previsao_consolidada_saude.Rda")
