...
df_base <- readr::read_rds("dados_p_graficos/base_veja.Rda")
...
data.frame(previsao_sarima = previsao$mean %>% as.numeric()) %>% 
  readr::write_rds("previsao-ajustados-sarima/veja.Rda")

previsao_veja <- previsao %>% 
  data.frame() %>% 
  cbind(teste)
...
data_previsao <- readr::read_rds("previsao-sessoes-sites/tbl_previsao_veja.Rda") %>%
...
data.frame(.index = data_previsao, 
  ...
  readr::write_rds("previsao-sessoes-sites/tbl_sarima_veja.Rda")
