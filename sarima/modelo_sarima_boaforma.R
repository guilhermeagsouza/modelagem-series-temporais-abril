...
previsao_boaforma %>% 
  readr::write_rds("dados-ajustados-teste/previsao-treino-sarima-boaforma.Rda")
...
fit.air2 <- forecast::Arima(
  y = stats::ts(
    data = df_w_outliers$y, frequency = 365.25/7, start = 2017 + 31/365.25
  ),
  order = c(1,1,1), 
  seasonal = c(0,0,0),
  method = "ML",
...
previsao_sarima_foradaamostra %>% 
  readr::write_rds("previsao-modelos-sarima/previsao_boaforma.Rda")
