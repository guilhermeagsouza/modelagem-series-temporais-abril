...
df_ajustados_STLF <- readr::read_rds("dados-ajustados-teste/previsao-treino-stlf-guiadoestudante.Rda") %>% 
  dplyr::rename(y_ajustado_modelo1 = PREVISAO_STLF)

df_ajustados_SARIMA <- readr::read_rds("dados-ajustados-teste/previsao-treino-sarima-guiadoestudante.Rda") %>% 
  dplyr::rename(y_ajustado_modelo2 = PREVISAO_SARIMA)

df_ajustados_PROPHET <- readr::read_rds("dados-ajustados-teste/previsao-treino-prophet-guiadoestudante.Rda") %>% 
...
pesos %>% 
  readr::write_rds("previsao-modelos-ensemble/pesos_ensemble_guiadoestudante.Rda")
################################################################################

previsao_stlf <- readr::read_rds("previsao-modelos-stlf/previsao_guiadoestudante.Rda")
previsao_sarima <- readr::read_rds("previsao-modelos-sarima/previsao_guiadoestudante.Rda")
previsao_prophet <- readr::read_rds("previsao-modelos-prophet/previsao_guiadoestudante.Rda")
...
modelo_ensemble %>% 
  readr::write_rds("previsao-modelos-ensemble/modelo_ensemble_guiadoestudante.Rda")
