...
df_ajustados_STLF <- readr::read_rds("dados-ajustados-teste/previsao-treino-stlf-claudia.Rda") %>% 
  dplyr::rename(y_ajustado_modelo1 = PREVISAO_STLF)

df_ajustados_SARIMA <- readr::read_rds("dados-ajustados-teste/previsao-treino-sarima-claudia.Rda") %>% 
  dplyr::rename(y_ajustado_modelo2 = PREVISAO_SARIMA)

df_ajustados_PROPHET <- readr::read_rds("dados-ajustados-teste/previsao-treino-prophet-claudia.Rda") %>% 
...
pesos <- df_ensemble %>% 
  slice(1)
pesos
pesos %>% readr::write_rds("previsao-modelos-ensemble/pesos_ensemble_claudia.Rda")

################################################################################

previsao_stlf <- readr::read_rds("previsao-modelos-stlf/previsao_claudia.Rda")
previsao_sarima <- readr::read_rds("previsao-modelos-sarima/previsao_claudia.Rda")
previsao_prophet <- readr::read_rds("previsao-modelos-prophet/previsao_claudia.Rda")
