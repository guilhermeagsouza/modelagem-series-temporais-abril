...
df_ajustados_STLF <- readr::read_rds("dados-ajustados-teste/previsao-treino-stlf-veja.Rda") %>% 
  dplyr::rename(y_ajustado_modelo1 = PREVISAO_STLF)

df_ajustados_SARIMA <- readr::read_rds("dados-ajustados-teste/previsao-treino-sarima-veja.Rda") %>% 
  dplyr::rename(y_ajustado_modelo2 = PREVISAO_SARIMA)

df_ajustados_PROPHET <- readr::read_rds("dados-ajustados-teste/previsao-treino-prophet-veja.Rda") %>% 
...
pesos %>% 
  readr::write_rds("previsao-modelos-ensemble/pesos_ensemble_veja.Rda")
################################################################################

previsao_stlf <- readr::read_rds("previsao-modelos-stlf/previsao_veja.Rda")
previsao_sarima <- readr::read_rds("previsao-modelos-sarima/previsao_veja.Rda")
...
modelo_ensemble %>% 
  readr::write_rds("previsao-modelos-ensemble/modelo_ensemble_veja.Rda")
