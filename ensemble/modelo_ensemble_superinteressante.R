...
df_ajustados_STLF <- readr::read_rds("dados-ajustados-teste/previsao-treino-stlf-superinteressante.Rda") %>% 
  dplyr::rename(y_ajustado_modelo1 = PREVISAO_STLF)

df_ajustados_SARIMA <- readr::read_rds("dados-ajustados-teste/previsao-treino-sarima-superinteressante.Rda") %>% 
  dplyr::rename(y_ajustado_modelo2 = PREVISAO_SARIMA)

df_ajustados_PROPHET <- readr::read_rds("dados-ajustados-teste/previsao-treino-prophet-superinteressante.Rda") %>% 
...
pesos %>% 
  readr::write_rds("previsao-modelos-ensemble/pesos_ensemble_superinteressante.Rda")
################################################################################

previsao_stlf <- readr::read_rds("previsao-modelos-stlf/previsao_superinteressante.Rda")
previsao_sarima <- readr::read_rds("previsao-modelos-sarima/previsao_superinteressante.Rda")
previsao_prophet <- readr::read_rds("previsao-modelos-prophet/previsao_superinteressante.Rda")
...
modelo_ensemble %>% 
  readr::write_rds("previsao-modelos-ensemble/modelo_ensemble_superinteressante.Rda")
